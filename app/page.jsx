...
function useSEO(page) {
  useEffect(() => {
    const titles = {
      home: 'UK Salary Calculator 2026-27 | Take-Home Pay | TaxdCalc',
      contractor: 'IR35 Contractor Calculator 2026-27 | TaxdCalc',
      nhs: 'NHS Pay Bands Take-Home 2026-27 | TaxdCalc',
      maternity: 'Statutory Maternity Pay Calculator 2026-27 | TaxdCalc',
      hourly: 'Hourly Rate Take-Home Calculator 2026-27 | TaxdCalc',
      bonus: 'Bonus Tax Calculator 2026-27 | TaxdCalc',
      sacrifice: 'Salary Sacrifice Pension Calculator 2026-27 | TaxdCalc',
      comparison: 'Job Offer Comparison Calculator 2026-27 | TaxdCalc',
      tools: 'All UK Tax Calculators 2026-27 | TaxdCalc',
      blog: 'UK Tax Guides 2026-27 | TaxdCalc',
    };
    if (typeof document !== 'undefined') {
      document.title = titles[page] || 'TaxdCalc';
    }
  }, [page]);
}

// ── TAX LOGIC 2026-27 (HMRC + Scottish Parliament confirmed) ──────────────────
const UK_BANDS = [
...
// ── FORMAT — literal £, no unicode escapes in JSX attrs ───────────────────────
// NOTE: fmtP / fmtPD use '\u00A3' inside JS string literals which IS correct.
// The bug was using it inside JSX string attributes (prefix="\u00A3") which renders literally.
function fmtP(n)  { return '\u00A3' + Math.max(0, Math.abs(n||0)).toLocaleString('en-GB',{maximumFractionDigits:0}); }
function fmtPD(n) { return '\u00A3' + Math.max(0,n||0).toLocaleString('en-GB',{minimumFractionDigits:2,maximumFractionDigits:2}); }
...
const ARTICLES = [
  {
    ...
    blocks:[
      ...
      {t:'p',v:'If you live in Scotland you pay Scottish Income Tax rates, with six bands instead of three. For 2026-27 the Starter band runs to 16,537 and Basic to 29,526. The Starter and Basic thresholds rose 7.4% this year.'},
      ...
      {t:'p',v:'If you earn 55,000: the first 12,570 is tax-free, the next 37,700 is taxed at 20%, and only the 4,730 above 50,270 is taxed at 40%. Effective rate: 17.1%, not 40%. A pay rise can never make you worse off.'},
      ...
      {t:'faq',q:'Does my tax code affect my personal allowance?',a:'Yes. The standard code 1257L means a 12,570 personal allowance. A different number changes the allowance. Use Advanced options in our calculator to enter your code.'},
    ]
  },
  {
    ...
    blocks:[
      ...
      {t:'p',v:'National Insurance is a tax on earnings. Class 1 contributions are deducted automatically from your pay through PAYE alongside income tax. Unlike income tax, NI is UK-wide. Scotland has no separate NI rates.'},
      {t:'h2',v:'Employee NI Rates 2026-27'},
      {t:'table',headers:['Earnings','Rate'],rows:[['Up to 12,570','0%'],['12,571 to 50,270','8%'],['Over 50,270','2%']]},
      {t:'p',v:'The NI rate drops from 8% to 2% above 50,270, unlike income tax which keeps increasing. Higher earners pay a lower effective NI rate than basic rate taxpayers.'},
      {t:'h2',v:'Employer NI: The Hidden Cost'},
      {t:'p',v:'Your employer pays 15% NI on your salary above 5,000 per year (increased from 13.8% in April 2025). For someone earning 40,000 that is an extra 5,250 on top of your salary. This is why salary sacrifice benefits both sides.'},
      {t:'h2',v:'Frequently Asked Questions'},
  ...
  },
  {
    ...
    blocks:[
      ...
      {t:'p',v:'HMRC tops up your pension with the income tax you would have paid on that money. A basic rate taxpayer contributing 80 gets 100 in their pension. A higher rate taxpayer contributing 60 gets 100.'},
      ...
      {t:'p',v:'Between 100,000 and 125,140 your Personal Allowance reduces, creating an effective 60% rate. A pension contribution bringing income below 100,000 restores your full allowance and can save over 2,000 immediately.'},
      {t:'h2',v:'Frequently Asked Questions'},
      {t:'faq',q:'Does salary sacrifice work differently in Scotland?',a:'Same mechanism, but Scottish taxpayers on the 42% Higher band save significantly more per pound contributed than equivalent earners in England.'},
  ...
  },
  {
    ...
    blocks:[
      ...
      {t:'table',headers:['Threshold','2026-27','Frozen Until'],rows:[['Personal Allowance','12,570','2031'],['Basic Rate limit','50,270','2031'],['Higher Rate limit','125,140','2031'],['NI Primary','12,570','2031'],['NI Upper Earnings Limit','50,270','2031']]},
      ...
      {t:'p',v:'Scottish taxpayers earning under approximately 33,500 pay slightly less income tax than equivalent earners in England. Above that level Scotland\'s higher rates mean more tax overall.'},
      ...
      {t:'p',v:'This makes Limited Company contractor structures less efficient than before. The dividend allowance remains at 500.'},
      {t:'h2',v:'Frequently Asked Questions'},
  ...
  return (
    <nav style={{background:C.navy,position:'sticky',top:0,zIndex:100,boxShadow:'0 2px 12px rgba(0,0,0,0.28)'}}>
      <div style={{maxWidth:1100,margin:'0 auto',padding:'0 20px',height:56,display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        <button onClick={()=>{nav('home');setOpen(false);}} style={{background:'none',border:'none',padding:0}}>
          <Logo light />
        </button>
        {mob ? (
...
// BACK BUTTON — prominent, navy background, clearly visible
function BackBtn({ nav, to='tools', label='Back to All Tools' }) {
  return (
    <button
      onClick={()=>nav(to)}
      style={{display:'inline-flex',alignItems:'center',gap:8,background:C.navy,color:'white',padding:'11px 20px',borderRadius:9,fontSize:13,fontWeight:700,border:'none',marginBottom:20,boxShadow:'0 2px 8px rgba(12,30,60,0.2)'}}
    >
...
// ── INPUT COMPONENTS ──────────────────────────────────────────────────────────
// CRITICAL FIX: prefix prop receives a string. In JSX string attributes,
// \u00A3 renders as literal text. Must use the actual £ character.
// All callers now pass the literal £ character directly.
  ...
  return (
    <div style={{marginBottom:18}}>
      ...
      <div style={{position:'relative'}}>
        {showPrefix&&(
          // Use literal £ — DO NOT use \u00A3 in JSX string attrs
          <span style={{position:'absolute',left:13,top:'50%',transform:'translateY(-50%)',color:C.slate,fontSize:16,fontWeight:600,fontFamily:'JetBrains Mono',pointerEvents:'none',lineHeight:1,zIndex:1}}>
            £
          </span>
        )}
        <input
          ...
          style={{
            width:'100%',
            // Extra left padding when showing £ prefix
            padding:showPrefix ? '13px 14px 13px 30px' : '13px 14px',
...
// Toggle switch component — clean, no emojis
  ...
  return (
    <button
      ...
      style={{
        ...
        borderRadius:13,
        border:'none',
        background:on ? C.scot : C.borderDark,
        position:'relative',
...
// Scotland question — clean, no flag emoji
function ScotlandToggle({ scotland, setScotland }) {
  return (
    <div style={{
      marginBottom:18,
      padding:'14px 16px',
      background:scotland ? C.scotBg : C.bg,
      border:`1.5px solid ${scotland ? C.scotBorder : C.borderDark}`,
      borderRadius:10,
      transition:'all 0.2s',
    }}>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:12}}>
        <div style={{minWidth:0}}>
          <div style={{fontSize:13,fontWeight:700,color:scotland ? C.scot : C.navyLight}}>
...
// Advanced options (tax code) — no emoji arrow, just text
      ...
      {show&&(
        <div style={{marginTop:10,padding:'14px 16px',background:C.bg,border:`1px solid ${C.border}`,borderRadius:10}}>
          <TextInput
            ...
            onChange={setTaxCode}
            placeholder="1257L"
            hint="Standard is 1257L"
          />
          {invalid&&(
            <div style={{fontSize:11,color:C.red,marginTop:-10,marginBottom:10}}>
              Unrecognised code — using default 12,570 allowance
            </div>
          )}
          <div style={{fontSize:11,color:C.slate,lineHeight:1.7}}>
            <strong>Common codes:</strong> 1257L = standard (12,570 allowance). BR = 20% on all income. D0 = 40% on all. 0T = no allowance. NT = no tax. K codes add to taxable income.
          </div>
        </div>
      )}
    </div>
...
// Result hero — fixed layout: clamp font, min-width 0, overflow hidden on number
function ResultHero({ label, value, monthly, pct, mob }) {
  return (
    <div style={{
      ...
      position:'relative',
      overflow:'hidden',
      boxShadow:'0 4px 20px rgba(12,30,60,0.22)',
    }}>
      ...
      <div style={{fontSize:10,color:'rgba(255,255,255,0.4)',letterSpacing:'0.12em',textTransform:'uppercase',fontFamily:'JetBrains Mono',marginBottom:6}}>
        {label||'Annual Take-Home Pay'}
      </div>
      {/* KEY FIX: min-width 0, overflow hidden, font clamp prevents overlap */}
      ...
      {pct!==undefined&&(
        <div style={{marginTop:14,display:'flex',alignItems:'center',gap:10,minWidth:0}}>
          ...
          <span style={{fontSize:12,color:C.tealLight,fontFamily:'JetBrains Mono',fontWeight:600,flexShrink:0}}>
            {Math.max(0,pct).toFixed(1)}% kept
          </span>
        </div>
      )}
    </div>
...
function HomePage({ nav }) {
  ...
  return (
    <main>
      ...
      {/* CALCULATOR — card floats over hero */}
      <div style={{maxWidth:1100,margin:mob?'-32px 0 0':'-44px auto 0',padding:mob?'0 16px':'0 24px'}}>
        <div style={{display:'grid',gridTemplateColumns:mob?'1fr':'minmax(280px,390px) 1fr',gap:20,alignItems:'start'}}>
          ...
          {/* INPUT PANEL — paddingTop ensures content not hidden behind hero overlap */}
          <div className="fu" style={{background:C.white,borderRadius:14,padding:22,boxShadow:C.shadow,border:`1px solid ${C.border}`}}>
            ...
            {/* FIXED: showPrefix prop instead of prefix string — renders literal £ in JSX */}
            <NumInput
              label="Annual Salary"
              value={salaryStr}
              onChange={setSalaryStr}
              showPrefix
              hint={salary>0 ? `${fmtP(salary/12)}/mo gross` : undefined}
            />
            ...
            <SelectField
              ...
              options={[
                ...
                {value:'plan1',label:'Plan 1 — pre Sept 2012  (threshold: 24,990)'},
            ...
            {/* Scotland question — clean, no emoji */}
            <ScotlandToggle scotland={scotland} setScotland={setScotland} />

            {/* Tax code — collapsed by default */}
            <TaxCodePanel taxCode={taxCode} setTaxCode={setTaxCode} />

            {/* Tax bands reference — updates with Scotland toggle */}
          ...
          <div className="fu" style={{display:'flex',flexDirection:'column',gap:14,minWidth:0}}>
            <ResultHero
              value={r.takeHome}
              monthly={r.monthly.takeHome}
              pct={salary>0 ? (r.takeHome/r.gross)*100 : 0}
              mob={mob}
            />
            ...
            <div style={{display:'flex',gap:4,background:C.white,padding:5,borderRadius:10,border:`1px solid ${C.border}`}}>
              {['annual','monthly','weekly','daily'].map(p=>(
                <button key={p} onClick={()=>setPeriod(p)} style={{flex:1,padding:'9px 4px',borderRadius:7,border:'none',background:period===p?C.teal:'transparent',color:period===p?'white':C.slate,fontSize:mob?11:12,fontWeight:period===p?600:400,textTransform:'capitalize'}}>
                  {p}
                </button>
              ))}
            </div>

            {/* Gross / Net — min-width 0 prevents overflow */}
            ...
            <Card style={{padding:mob?16:'20px 22px'}}>
              <div style={{display:'flex',gap:2,marginBottom:16,borderBottom:`1px solid ${C.border}`}}>
                {['breakdown','table'].map(t=>(
                  <button key={t} onClick={()=>setTab(t)} style={{padding:'7px 14px',border:'none',background:'transparent',fontSize:13,color:tab===t?C.navy:C.slate,fontWeight:tab===t?700:400,borderBottom:tab===t?`2px solid ${C.teal}`:'2px solid transparent',marginBottom:-1,textTransform:'capitalize'}}>
                    {t}
                  </button>
                ))}
              </div>
              ...
              {tab==='table'&&(
                <div style={{overflowX:'auto',WebkitOverflowScrolling:'touch'}}>
                  <table style={{width:'100%',borderCollapse:'collapse',fontSize:12,minWidth:300}}>
                    <thead>
                      <tr>{['','Annual','Monthly','Weekly'].map(h=><th key={h} style={{textAlign:h?'right':'left',padding:'6px',color:C.slate,fontSize:10,textTransform:'uppercase',borderBottom:`1px solid ${C.border}`}}>{h}</th>)}</tr>
                    </thead>
                    <tbody>
                      {[
      ...
      <section style={{background:C.white,borderTop:`1px solid ${C.border}`,padding:mob?'44px 16px':'52px 24px',marginTop:48}}>
        <div style={{maxWidth:1100,margin:'0 auto'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:mob?'1fr 1fr':'repeat(auto-fill,minmax(178px,1fr))',gap:12}}>
            {[
              {icon:'🏗️',label:'IR35 Contractor',sub:'PAYE vs Ltd Co',tag:'Popular',page:'contractor'},
              {icon:'⏰',label:'Hourly Rate',sub:'Hourly to annual',page:'hourly'},
              ...
              {icon:'🏦',label:'Salary Sacrifice',sub:'Pension real cost',page:'sacrifice'},
              {icon:'📊',label:'Job Comparison',sub:'Compare 2 offers',page:'comparison'},
              {icon:'🌍',label:'All Tools',sub:'View everything',page:'tools'},
            ].map(c=>(
    ...
    <main>
      ...
      <div style={{maxWidth:860,margin:mob?'-28px 0 0':'-34px auto 0',padding:mob?'0 16px 48px':'0 24px 56px'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:mob?'1fr':'1fr 1fr',gap:14,marginBottom:14}}>
          {[
            {label:'Inside IR35 (PAYE)',value:rIn.takeHome,sub:'All income taxed as employment income.',color:C.red,bg:C.redBg,border:C.redBorder},
            {label:'Outside IR35 (Ltd Co.)',value:limitedTH,sub:'Salary + dividends. Basic rate dividend tax now 10.75% from April 2026.',color:C.teal,bg:C.tealBg,border:C.tealBorder},
          ].map(s=>(
        ...
        {annual>0&&(
          <div style={{background:saving>0?C.tealBg:C.redBg,border:`1.5px solid ${saving>0?C.tealBorder:C.redBorder}`,borderRadius:10,padding:'14px 18px',marginBottom:14}}>
            <span style={{fontSize:14,color:C.text,lineHeight:1.5}}>
              Outside IR35 {saving>0?'saves you':'costs you'} <strong style={{color:saving>0?C.teal:C.red}}>{fmtP(Math.abs(saving))}/year</strong> ({fmtP(Math.abs(saving/12))}/month) {saving>0?'more':'less'} in take-home pay
            </span>
          </div>
        )}
        ...
        <div style={{background:C.amberBg,border:`1px solid ${C.amberBorder}`,borderRadius:10,padding:'13px 17px',marginBottom:20}}>
          ...
          <div style={{fontSize:12,color:'#78350F',lineHeight:1.6}}>IR35 status depends on your actual working practices. This is indicative only. Seek advice from a qualified IR35 specialist. Dividend tax rates increased in April 2026.</div>
        </div>
        ...
        <div style={{padding:'14px 16px',background:C.bg,border:`1px solid ${C.border}`,borderRadius:10,display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:10}}>
          <span style={{fontSize:13,color:C.textMid}}>Also need your PAYE take-home pay?</span>
          <button onClick={()=>nav('home')} style={{background:C.teal,color:'white',padding:'9px 18px',borderRadius:7,fontWeight:700,fontSize:13,border:'none'}}>
      ...
      <div style={{maxWidth:960,margin:mob?'-28px 0 0':'-34px auto 0',padding:mob?'0 16px 48px':'0 24px 56px'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:mob?'1fr':'1fr 1fr',gap:20,alignItems:'start',marginBottom:20}}>
          <Card>
            ...
            <ScotlandToggle scotland={scotland} setScotland={setScotland} />
            <div style={{padding:'10px 13px',background:C.tealBg,border:`1px solid ${C.tealBorder}`,borderRadius:7,fontSize:11,color:'#0f766e',lineHeight:1.6}}>
              NHS pension contribution rates range from 5% to 14.5% depending on your pensionable pay tier.
            </div>
          </Card>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:14,minWidth:0}}>
            ...
            <Card>
              {[
                {l:'Gross salary',v:r.gross},
                {l:`Income Tax${scotland?' (Scottish)':''}`,v:r.incomeTax,neg:true},
                {l:'National Insurance',v:r.ni,neg:true},
                {l:`NHS Pension (${pension}%)`,v:r.pension,neg:true},
                {l:'Annual take-home',v:r.takeHome,big:true,color:C.teal},
              ].map((row,i)=>(
                <div key={row.l} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'10px 0',borderBottom:i<4?`1px solid ${C.border}`:'none'}}>
                  ...
                  <span style={{fontFamily:'JetBrains Mono',fontSize:row.big?16:13,color:row.neg?C.red:(row.color||C.navy),fontWeight:row.big?700:400}}>
                    {row.neg?'-':''}{fmtP(row.v)}
                  </span>
                </div>
              ))}
            ...
            <table style={{width:'100%',borderCollapse:'collapse',fontSize:13,minWidth:480}}>
              ...
              <tbody>
                {NHS_BANDS.map((b,i)=>{
                  ...
                  return (
                    <tr key={b.band} onClick={()=>{setSelectedBand(b.band);setSpineIdx(0);window.scrollTo({top:0,behavior:'smooth'});}}
                      style={{background:isSel?C.tealBg:i%2===0?C.white:C.bg,cursor:'pointer',borderBottom:`1px solid ${C.border}`}}>
        ...
        <div style={{marginTop:16,padding:'14px 16px',background:C.bg,border:`1px solid ${C.border}`,borderRadius:10,display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:10}}>
...
function MaternityPage({ nav }) {
  ...
  return (
    <main>
      <PageHero tag="Maternity Pay 2026-27" title="Statutory Maternity Pay<br /><em style='color:#14B8A6'>Calculator</em>" sub="SMP entitlement and weekly breakdown. Statutory rate: 187.18 per week for 2026-27." mob={mob} />
      <div style={{maxWidth:860,margin:mob?'-28px 0 0':'-34px auto 0',padding:mob?'0 16px 48px':'0 24px 56px'}}>
        <BackBtn nav={nav} />
        <div style={{display:'grid',gridTemplateColumns:mob?'1fr':'1fr 1fr',gap:20,alignItems:'start'}}>
          <Card>
            ...
            <div style={{marginBottom:18}}>
              ...
              <div style={{display:'flex',gap:10}}>
                {[['Yes',true],['No — SMP only',false]].map(([lbl,val])=>(
                  <button key={String(val)} onClick={()=>setEnhanced(val)} style={{flex:1,padding:'10px',borderRadius:8,border:`1.5px solid ${enhanced===val?C.teal:C.borderDark}`,background:enhanced===val?C.tealBg:C.white,color:enhanced===val?C.teal:C.textMid,fontSize:13,fontWeight:enhanced===val?700:400}}>
                    {lbl}
                  </button>
                ))}
              </div>
            ...
            <div style={{padding:'12px 14px',background:C.amberBg,border:`1px solid ${C.amberBorder}`,borderRadius:8,fontSize:12,color:'#78350F',lineHeight:1.65,marginTop:8}}>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:14,minWidth:0}}>
            ...
            {salary>0&&(
              <Card>
                <div style={{fontSize:11,fontWeight:700,color:C.navy,textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:12}}>Normal Pay vs Maternity Pay</div>
                {[{l:'Normal weekly',v:weeklyGross,pct:100,color:C.navy},{l:'Wks 1-6 (90%)',v:phase1,pct:90,color:C.teal},{l:'Wks 7-39 (SMP)',v:smpRate,pct:Math.min(100,(smpRate/weeklyGross)*100),color:C.amber}].map(row=>(
                  <div key={row.l} style={{marginBottom:12}}>
                    <div style={{display:'flex',justifyContent:'space-between',marginBottom:4}}>
                      <span style={{fontSize:12,color:C.textMid}}>{row.l}</span>
                      <span style={{fontFamily:'JetBrains Mono',fontSize:13,color:row.color,fontWeight:600}}>{fmtPD(row.v)}</span>
                    </div>
        ...
        <div style={{marginTop:16,padding:'14px 16px',background:C.bg,border:`1px solid ${C.border}`,borderRadius:10,display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:10}}>
          <span style={{fontSize:13,color:C.textMid}}>Need your normal take-home pay?</span>
          <button onClick={()=>nav('home')} style={{background:C.teal,color:'white',padding:'9px 18px',borderRadius:7,fontWeight:700,fontSize:13,border:'none'}}>Open Salary Calculator</button>
        </div>
      </div>
    </main>
      ...
      <div style={{maxWidth:860,margin:mob?'-28px 0 0':'-34px auto 0',padding:mob?'0 16px 48px':'0 24px 56px'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:mob?'1fr':'1fr 1fr',gap:20,alignItems:'start'}}>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:14,minWidth:0}}>
            ...
            <Card>
              {[
                {l:'Gross hourly rate',v:hourly},
                {l:'Net hourly rate',v:netHourly,color:C.teal,big:true},
                {l:'Annual gross',v:annual},
                {l:'Annual take-home',v:r.takeHome,color:C.teal,big:true},
                {l:'Monthly take-home',v:r.monthly.takeHome},
                {l:'Weekly take-home',v:r.weekly.takeHome},
              ].map((row,i)=>(
        ...
        <div style={{display:'grid',gridTemplateColumns:mob?'1fr':'1fr 1fr',gap:20,alignItems:'start'}}>
          <Card>
            ...
            <div style={{padding:'12px 14px',background:C.tealBg,border:`1px solid ${C.tealBorder}`,borderRadius:8,fontSize:12,color:'#0f766e',lineHeight:1.65}}>
              Your bonus is taxed at your marginal rate. If salary + bonus crosses a band boundary, part is taxed at the higher rate. {scotland&&'Scottish rates apply.'}
            </div>
          </Card>
          <div style={{display:'flex',flexDirection:'column',gap:14,minWidth:0}}>
            <div style={{background:`linear-gradient(135deg,${C.navy},${C.navyMid})`,borderRadius:14,padding:mob?20:24,boxShadow:C.shadow}}>
              ...
              <div style={{fontFamily:'DM Serif Display',fontSize:'clamp(26px,5vw,44px)',color:'white',lineHeight:1,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>
                <AnimNum value={netBonus} />
              </div>
            ...
            <Card>
              {[
                {l:'Gross bonus',v:bonus},
                {l:'Extra income tax',v:extraTax,neg:true},
                {l:'Extra NI',v:extraNI,neg:true},
                {l:'Net bonus (you receive)',v:netBonus,big:true,color:C.teal},
              ].map((row,i)=>(
              ...
              <div style={{marginTop:12,height:7,background:C.border,borderRadius:4,overflow:'hidden'}}>
                <div className="bfill" style={{width:Math.min(100,pct)+'%',height:'100%',background:`linear-gradient(90deg,${C.teal},${C.tealLight})`}} />
              </div>
            </Card>
          </div>
        </div>
...
function SacrificePage({ nav }) {
  ...
  return (
    <main>
      <PageHero tag="Salary Sacrifice" title="Salary Sacrifice<br /><em style='color:#14B8A6'>Pension Calculator</em>" sub="The real cost of a pension contribution after tax and NI savings. Scottish higher rates mean even bigger savings." mob={mob} />
      <div style={{maxWidth:860,margin:mob?'-28px 0 0':'-34px auto 0',padding:mob?'0 16px 48px':'0 24px 56px'}}>
        <BackBtn nav={nav} />
        <div style={{display:'grid',gridTemplateColumns:mob?'1fr':'1fr 1fr',gap:20,alignItems:'start'}}>
          <Card>
            ...
            <div style={{padding:'12px 14px',background:C.tealBg,border:`1px solid ${C.tealBorder}`,borderRadius:8,fontSize:12,color:'#0f766e',lineHeight:1.7}}>
              Salary sacrifice reduces gross salary before tax AND NI are calculated — saving both. A personal pension only recovers income tax.
            </div>
          </Card>
          <div style={{display:'flex',flexDirection:'column',gap:14,minWidth:0}}>
            <div style={{background:`linear-gradient(135deg,${C.navy},${C.navyMid})`,borderRadius:14,padding:mob?20:24,boxShadow:C.shadow}}>
              ...
              <div style={{fontFamily:'DM Serif Display',fontSize:'clamp(26px,5vw,44px)',color:'white',lineHeight:1,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>
                <AnimNum value={netCost} />
              </div>
              <div style={{fontSize:13,color:C.tealLight,fontFamily:'JetBrains Mono',marginTop:5}}>vs {fmtP(amt)} gross contribution</div>
            </div>
            <Card>
              {[
                {l:'Gross contribution',v:amt},
                {l:'Income tax saved',v:taxSaving,pos:true},
                {l:'NI saved',v:niSaving,pos:true},
                {l:'Real cost to you',v:netCost,big:true,color:C.teal},
                {l:'Take-home reduction',v:thDiff,neg:true},
              ].map((row,i)=>(
                <div key={row.l} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'9px 0',borderBottom:i<4?`1px solid ${C.border}`:'none'}}>
                  <span style={{fontSize:row.big?14:13,fontWeight:row.big?700:400,color:row.big?C.text:C.textMid}}>{row.l}</span>
                  <span style={{fontFamily:'JetBrains Mono',fontSize:row.big?16:13,color:row.neg?C.red:row.pos?C.green:(row.color||C.navy),fontWeight:row.big?700:400}}>
                    {row.pos?'+':row.neg?'-':''}{fmtP(row.v)}
                  </span>
                </div>
              ))}
            </Card>
...
function ComparisonPage({ nav }) {
  ...
  const JobPanel = ({ job, setJob, color, bg, border, r, idx }) => (
    <div>
      ...
      <div style={{background:bg,border:`2px solid ${border}`,borderRadius:12,padding:'18px',textAlign:'center',minWidth:0}}>
        ...
        <div style={{fontFamily:'DM Serif Display',fontSize:'clamp(24px,4vw,34px)',color:C.navy,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>
          <AnimNum value={r.takeHome} />
        </div>
  ...
  return (
    <main>
      <PageHero tag="Job Comparison" title="Job Offer<br /><em style='color:#14B8A6'>Comparison Calculator</em>" sub="Compare two packages side by side. Compare a Scotland job vs an England job." mob={mob} />
      <div style={{maxWidth:1000,margin:mob?'-28px 0 0':'-34px auto 0',padding:mob?'0 16px 48px':'0 24px 56px'}}>
        ...
        <Card style={{textAlign:'center',padding:'22px',marginBottom:14}}>
          ...
          <div style={{fontFamily:'DM Serif Display',fontSize:mob?30:40,color:diff>=0?C.teal:C.red,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>
            {diff>=0?'+':''}{fmtP(diff)}
          </div>
...
function ToolsPage({ nav }) {
  ...
  return (
    <main>
      <PageHero tag="TaxdCalc" title="All Calculators" sub="8 live UK tax calculators. Scotland supported throughout. More coming soon." mob={mob} />
    ...
    <main style={{background:C.bg,minHeight:'100vh'}}>
      ...
      <div style={{maxWidth:1000,margin:'-32px auto 60px',padding:'0 20px'}}>
        <div style={{display:'grid',gridTemplateColumns:mob?'1fr':'repeat(auto-fill,minmax(280px,1fr))',gap:20}}>
          {ARTICLES.map(a=>(
            <article key={a.slug}>
              <button onClick={()=>nav('article-'+a.slug)} style={{background:C.white,borderRadius:12,padding:22,boxShadow:C.shadow,border:`1px solid ${C.border}`,textAlign:'left',width:'100%',display:'block'}}>
...
function ArticlePage({ nav, slug }) {
  ...
  useEffect(()=>{ if(article&&typeof document!=='undefined') document.title=article.title+' | TaxdCalc'; },[article]);
  ...
  function renderBlock(block, i) {
    ...
    if (block.t==='h3') return <h3 key={i} style={{fontFamily:'DM Serif Display',fontSize:18,color:C.navy,marginTop:22,marginBottom:8}}>{block.v}</h3>;
    ...
    <main style={{background:C.bg,minHeight:'100vh'}}>
      ...
      <nav aria-label="Breadcrumb" style={{background:C.navy,padding:'0 24px'}}>
        <div style={{maxWidth:800,margin:'0 auto',height:46,display:'flex',alignItems:'center',gap:8,fontSize:13}}>
          <button onClick={()=>nav('home')} style={{background:'none',border:'none',color:'rgba(255,255,255,0.45)',fontSize:13,padding:0,cursor:'pointer'}}>TaxdCalc</button>
      ...
      <div style={{maxWidth:800,margin:'0 auto',padding:'clamp(20px,4vw,44px) 20px'}}>
        ...
        <div style={{background:`linear-gradient(135deg,${C.navy},${C.navyMid})`,borderRadius:14,padding:mob?22:32,marginTop:22,textAlign:'center'}}>
          ...
          <p style={{color:'rgba(255,255,255,0.48)',fontSize:14,marginBottom:18}}>
            Free, accurate, 2026-27 rates. Scotland and tax code supported. No sign-up.
          </p>
          <div style={{display:'flex',gap:12,justifyContent:'center',flexWrap:'wrap'}}>
            <button onClick={()=>nav('home')} style={{background:C.teal,color:'white',padding:'12px 24px',borderRadius:8,fontWeight:700,fontSize:14,border:'none',cursor:'pointer'}}>
              Salary Calculator
            </button>
            {article.relatedCalc&&article.relatedCalc.page!=='home'&&(
              <button onClick={()=>nav(article.relatedCalc.page)} style={{background:'rgba(255,255,255,0.12)',color:'white',padding:'12px 24px',borderRadius:8,fontWeight:700,fontSize:14,border:'1px solid rgba(255,255,255,0.2)',cursor:'pointer'}}>
        ...
        {others.length>0&&(
          <section style={{marginTop:30}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:mob?'1fr':'repeat(auto-fill,minmax(220px,1fr))',gap:12}}>
              {others.map(a=>(
                <article key={a.slug}>
                  <button onClick={()=>nav('article-'+a.slug)} style={{background:C.white,borderRadius:10,padding:16,border:`1px solid ${C.border}`,textAlign:'left',boxShadow:C.shadow,width:'100%',cursor:'pointer'}}>
