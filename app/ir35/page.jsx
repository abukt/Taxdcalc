...
export default function IR35Page(){
  ...
  const annual=Math.max(0,(Number(dayRate)||0)*days);
    ...
    <>
      ...
      <div className="ai-answer" style={{background:C.tealBg,borderBottom:`1px solid ${C.tealBd}`,padding:mob?'14px 16px':'16px 24px'}}>
        <div style={{maxWidth:900,margin:'0 auto'}}>
          ...
          <div style={{marginTop:10,fontSize:12,color:'#0f766e',lineHeight:1.6}}>
            💡 <strong>April 2026:</strong> Dividend tax rose to 10.75% basic rate. At rates below ~£600/day, inside IR35 now often beats outside IR35.
          </div>
        </div>
      </div>
      ...
      <div style={{background:`linear-gradient(135deg,${C.navy},${C.navyMid})`,padding:mob?'24px 20px 38px':'32px 24px 46px',position:'relative',overflow:'hidden'}}>
        ...
        <div style={{maxWidth:900,margin:'0 auto'}}>
          ...
          <p style={{color:'rgba(255,255,255,0.5)',fontSize:mob?13:15,maxWidth:540}}>Compare PAYE inside IR35 vs Limited Company outside IR35 for any day rate.</p>
        </div>
      </div>
        ...
        <div style={{background:C.white,borderRadius:14,padding:mob?18:24,boxShadow:C.shadow,border:`1px solid ${C.border}`,marginBottom:16}} className="fi">
          ...
          <div style={{display:'grid',gridTemplateColumns:mob?'1fr':'1fr 1fr 1fr',gap:14,marginBottom:18}}>
            ...
            <div>
              ...
              <input type="number" inputMode="decimal" value={days} onChange={e=>setDays(Math.max(1,Math.min(260,Number(e.target.value))))} min={1} max={260} style={{width:'100%',padding:'12px 14px',border:`1.5px solid ${C.borderDk}`,borderRadius:8,fontSize:16,fontFamily:'JetBrains Mono',fontWeight:600,color:C.navy,background:'white',outline:'none'}}/>
            </div>
            <div>
              <div style={{display:'flex',justifyContent:'space-between',marginBottom:8}}><label style={{fontSize:12,fontWeight:600,color:C.navy}}>Pension (inside)</label><span style={{fontFamily:'JetBrains Mono',fontSize:12,color:C.teal,fontWeight:600}}>{penPct}%</span></div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:14}}>
            <div style={{background:`linear-gradient(135deg,${C.navy},${C.navyMid})`,borderRadius:12,padding:'18px 16px'}}>
              <div style={{fontSize:10,color:'rgba(255,255,255,0.45)',textTransform:'uppercase',fontFamily:'JetBrains Mono',marginBottom:4}}>Inside IR35 (PAYE)</div>
              ...
              <div style={{marginTop:12,fontSize:11,color:'rgba(255,255,255,0.35)',lineHeight:1.9}}>
                <div>IT: -{fmt(inside.it)}</div><div>NI: -{fmt(inside.ni)}</div>
              </div>
            </div>
            <div style={{background:outsideWins?`linear-gradient(135deg,#064E3B,#065F46)`:`linear-gradient(135deg,#1E1B4B,#312E81)`,borderRadius:12,padding:'18px 16px'}}>
              <div style={{fontSize:10,color:'rgba(255,255,255,0.45)',textTransform:'uppercase',fontFamily:'JetBrains Mono',marginBottom:4}}>Outside IR35 (Ltd)</div>
              <div style={{fontFamily:'DM Serif Display',fontSize:mob?26:34,color:'white',lineHeight:1}}><AnimNum value={outside.takeHome}/></div>
              <div style={{marginTop:12,fontSize:11,color:'rgba(255,255,255,0.35)',lineHeight:1.9}}>
                <div>Corp: -{fmt(outside.corp)}</div><div>Div Tax: -{fmt(outside.bDiv+outside.hDiv)}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{background:C.white,borderRadius:12,padding:mob?16:22,border:`1px solid ${C.border}`,boxShadow:C.shadow,marginBottom:16}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:mob?'1fr':'1fr 1fr',gap:14}}>
            <div style={{background:C.bg,borderRadius:10,padding:'14px 16px'}}>
              ...
              <div style={{fontSize:12, color:C.mid}}>Standard employment tax (Income Tax & NI) after pension contributions.</div>
            </div>
            <div style={{background:C.bg,borderRadius:10,padding:'14px 16px'}}>
              <div style={{fontSize:12,fontWeight:700,color:C.navy,marginBottom:10}}>Outside IR35 — Ltd Co.</div>
              <div style={{fontSize:12, color:C.mid}}>£12,570 Salary + Dividends after 19% Corp Tax and new 10.75% basic dividend tax.</div>
            </div>
          </div>
        </div>

        <div style={{background:C.white,borderRadius:12,padding:mob?16:22,border:`1px solid ${C.border}`,boxShadow:C.shadow,marginBottom:16}}>
          <h2 style={{fontFamily:'DM Serif Display',fontSize:mob?17:20,color:C.navy,marginBottom:4}}>IR35 FAQ</h2>
          {[
            {q:'What is the take-home pay inside IR35 on a £500/day contract?',a:'On £500/day at 220 days (£110,000/year) inside IR35, take-home is approximately £70,157/year.'},
            {q:'Is it better to be inside or outside IR35 in 2026-27?',a:'At rates below ~£600/day, inside IR35 often produces more take-home due to higher dividend taxes.'}
      ...
      {mob && annual > 0 && (
        <div style={{position:'fixed',bottom:0,left:0,right:0,background:C.navy,zIndex:90,height:52,display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0 20px',boxShadow:'0 -2px 16px rgba(0,0,0,0.3)'}}>
          <div>
            <div style={{fontSize:9,color:'rgba(255,255,255,0.4)',fontFamily:'JetBrains Mono'}}>Inside</div>
            <div style={{fontFamily:'DM Serif Display',fontSize:19,color:'#14B8A6'}}>{fmt(inside.takeHome)}</div>
          </div>
          <div style={{textAlign:'right'}}>
            <div style={{fontSize:9,color:'rgba(255,255,255,0.4)',fontFamily:'JetBrains Mono'}}>Outside</div>
            <div style={{fontFamily:'JetBrains Mono',fontSize:13,color:'white',fontWeight:700}}>{fmt(outside.takeHome)}</div>
          </div>
        </div>
      )}
      <Footer/>
