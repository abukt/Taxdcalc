...
const C={
  ...
  amber:'#D97706',amberBg:'#FFFBEB',amberBorder:'#FDE68A',border:'#E2E8F0',borderDark:'#CBD5E1',
  white:'#FFFFFF',text:'#1E293B',textMid:'#475569',slate:'#64748B',slateLight:'#94A3B8',
...
export default function MaternityPage() {
  ...
  const schemaFAQ={'@context':'https://schema.org','@type':'FAQPage',mainEntity:[
    {'@type':'Question',name:'How much is Statutory Maternity Pay (SMP) in 2026-27?',acceptedAnswer:{'@type':'Answer',text:'SMP in 2026-27 is: 90% of average weekly earnings for the first 6 weeks, then the lower of £184.03/week or 90% of earnings for weeks 7 to 39 (33 weeks). Total minimum SMP over 39 weeks is approximately £6,745 for lower earners. Higher earners receive more in the first 6 weeks.'}},
    {'@type':'Question',name:'How long does maternity pay last?',acceptedAnswer:{'@type':'Answer',text:'Statutory Maternity Pay (SMP) lasts up to 39 weeks. Maternity leave can last up to 52 weeks but only the first 39 weeks are paid if you qualify for SMP. Weeks 40–52 are unpaid unless your employer offers enhanced maternity pay.'}},
    {'@type':'Question',name:'Is maternity pay taxable in the UK?',acceptedAnswer:{'@type':'Answer',text:'Yes. Statutory Maternity Pay (SMP) is taxable income. Income tax and National Insurance are deducted by your employer through PAYE in the normal way. The personal allowance (£12,570 in 2026-27) still applies, so lower earners may pay no income tax during maternity leave.'}},
    {'@type':'Question',name:'Do I qualify for Statutory Maternity Pay?',acceptedAnswer:{'@type':'Answer',text:'To qualify for SMP you must: have worked for the same employer continuously for at least 26 weeks into the 15th week before your due date, earn at least £123/week (Lower Earnings Limit 2026-27), and give the correct notice. If you do not qualify for SMP you may qualify for Maternity Allowance from the government instead.'}},
    {'@type':'Question',name:'Can I get maternity pay if I am self-employed?',acceptedAnswer:{'@type':'Answer',text:'Self-employed people do not qualify for Statutory Maternity Pay (SMP) as that is paid by employers. Instead, you may claim Maternity Allowance directly from the government if you have been self-employed and paying Class 2 NI contributions for at least 26 weeks in the 66 weeks before your due date. The Maternity Allowance rate for 2026-27 is up to £184.03/week for 39 weeks.'}},
  ]};
  const schemaBreadcrumb={'@context':'https://schema.org','@type':'BreadcrumbList',itemListElement:[{'@type':'ListItem',position:1,name:'TaxdCalc',item:'https://taxdcal.co.uk'},{'@type':'ListItem',position:2,name:'Maternity Pay Calculator',item:'https://taxdcal.co.uk/maternity'}]};
  const schemaCalc={'@context':'https://schema.org','@type':'WebApplication',name:'UK Maternity Pay Calculator 2026-27',applicationCategory:'FinanceApplication',operatingSystem:'Any',offers:{'@type':'Offer',price:'0',priceCurrency:'GBP'},provider:{'@type':'Organization',name:'TaxdCalc',url:'https://taxdcal.co.uk'},url:'https://taxdcal.co.uk/maternity'};
    ...
    <>
      ...
      {salary > 0 && (
        <div className="ai-answer" style={{background:'#F0FDFA',borderBottom:'1px solid #99F6E4',padding:mob?'13px 16px':'15px 24px'}}>
          <div style={{maxWidth:1000,margin:'0 auto'}}>
            <div style={{fontSize:10,color:'#0D9488',fontWeight:700,letterSpacing:'0.12em',textTransform:'uppercase',fontFamily:'JetBrains Mono',marginBottom:5}}>Quick Answer — SMP 2026-27</div>
            <p style={{fontSize:mob?13:14,color:'#0f766e',fontWeight:600,lineHeight:1.6}}>
              On a {'£'}{salary.toLocaleString('en-GB')} salary: first 6 weeks at 90% = <strong>{'£'}{Math.round(phase1*6).toLocaleString('en-GB')}</strong>, then 33 weeks at {'£'}187.18/week = <strong>{'£'}{Math.round(phase2*33).toLocaleString('en-GB')}</strong>. Total SMP: <strong>{'£'}{Math.round(smpTotal).toLocaleString('en-GB')}</strong> over 39 weeks.
            </p>
          </div>
        </div>
      )}
        ...
        <div style={{display:'grid',gridTemplateColumns:mob?'1fr':'1fr 1fr',gap:20,alignItems:'start'}}>
          <div style={{background:'white',borderRadius:14,padding:mob?20:26,boxShadow:C.shadow,border:'1px solid '+C.border}} className="fu">
            ...
            <div style={{padding:'12px 14px',background:'#FFFBEB',border:'1px solid #FDE68A',borderRadius:8,fontSize:12,color:'#78350F',lineHeight:1.65}}>
              To qualify for SMP: employed for 26 weeks by week 15 before due date, average earnings at least {'\u00A3'}123/week.
            </div>
          </div>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:14}} className="fu">
            ...
            <div style={{background:'white',borderRadius:12,padding:mob?16:22,border:'1px solid '+C.border,boxShadow:C.shadow}}>
              ...
              {[
                ...
                {label:'Weeks 40 to 52',sub:'Unpaid (if taking full leave)',weekly:0,weeks:13,color:C.slateLight},
              ].map(p=>(
      ...
      {mob && salary > 0 && (
        <div style={{position:'fixed',bottom:0,left:0,right:0,background:C.navy,zIndex:90,height:52,display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0 20px',boxShadow:'0 -2px 16px rgba(0,0,0,0.3)'}}>
          <div>
            <div style={{fontSize:9,color:'rgba(255,255,255,0.4)',fontFamily:'JetBrains Mono',textTransform:'uppercase',letterSpacing:'0.1em'}}>Total SMP</div>
            <div style={{fontFamily:'DM Serif Display',fontSize:19,color:'#14B8A6',lineHeight:1}}>{fmt(smpTotal)}</div>
          </div>
          <div style={{textAlign:'right'}}>
            <div style={{fontSize:9,color:'rgba(255,255,255,0.4)',fontFamily:'JetBrains Mono',textTransform:'uppercase',letterSpacing:'0.1em'}}>Weekly SMP</div>
            <div style={{fontFamily:'JetBrains Mono',fontSize:13,color:'white',fontWeight:700,lineHeight:1}}>{fmtD(smpTotal/39)}</div>
          </div>
        </div>
      )}
      <Footer/>
